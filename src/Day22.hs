...
recursiveCombat p1 p2 prev1 prev2
  ...
  | head p1 < length p1 && head p2 < length p2 =
    if subgame sub1 sub2 [] []
      then recursiveCombat
             (tail p1 ++ [head p1, head p2])
             (tail p2)
             (p1 : prev1)
             (p2 : prev2)
      else recursiveCombat
             (tail p1)
             (tail p2 ++ [head p2, head p1])
             (p1 : prev1)
             (p2 : prev2)
  | head p1 > head p2 =
    recursiveCombat
...
subgame p3 p4 prev1 prev2
  ...
  | head p3 < length p3 && head p4 < length p4 =
    if subgame sub1 sub2 [] []
      then subgame
             (tail p3 ++ [head p3, head p4])
             (tail p4)
             (p3 : prev1)
             (p4 : prev2)
      else subgame
             (tail p3)
             (tail p4 ++ [head p4, head p3])
             (p3 : prev1)
             (p4 : prev2)
  | head p3 > head p4 =
    subgame (tail p3 ++ [head p3, head p4]) (tail p4) (p3 : prev1) (p4 : prev2)
  | otherwise =
    subgame (tail p3) (tail p4 ++ [head p4, head p3]) (p3 : prev1) (p4 : prev2)
  where
    sub1 = take (head p3) $ tail p3
