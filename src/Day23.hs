...
newlist list =
  takeWhile (/= destination list) (tail list') ++
  [destination list] ++
  removed list ++
  drop 1 (dropWhile (/= destination list) (tail list')) ++ [head list]
  where
...
solutionDay23a =
  putStrLn $
  concatMap show $ answer $ iterate newlist (toDigitList input23) !! 100
