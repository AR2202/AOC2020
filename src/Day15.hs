...
addNext list = list ++ [next]
  where
    next
      | last list `elem` init list =
        1 + (length $ takeWhile (/= last list) $ reverse $init list)
      | otherwise = 0

number2020 initialNums =
  last $ (iterate addNext initialNums) !! (2020 - length initialNums)
...
insertNext !intmap !prev ind = (IM.insert prev ind intmap, next)
  where
    next =
      case (IM.lookup prev intmap) of
        Nothing -> 0
        Just x  -> ind - x

insertAll !startlist n =
  L.foldl'
    (uncurry insertNext)
    (startmap, last startlist)
    [length startlist .. n - 1]
  where
    !startmap = IM.fromList $ zip (init startlist) [1 ..]
