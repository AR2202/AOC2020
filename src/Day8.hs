...
runInstructions prog progstate
  | currIndex progstate `elem` previouslyRun progstate =
    Left $ currAcc progstate
...
changeNthelem list n = (take n list) ++ changed : (drop (n + 1) list)
  where
    changed = changeInstructionLine $list !! n

allPrograms prog =
  [ changeNthelem prog n
  | n <- [0 .. length prog - 1]
  , fst (prog !! n) == NOP || fst (prog !! n) == JMP
  ]

findTerminating proglist =
  case (runInstructions (head proglist) initialState) of
    (Right x) -> x
