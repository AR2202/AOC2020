...
allAddresses mask string = generatePossible (Prelude.map fromJust $ applyMask' mask string) [""]

readWithMask mask string = masked mask $ i36tobinary $ read string

maskedValues (masklist, list) = Prelude.map (readWithMask (masklist !! 1) . (!! 1)) list

memAddresses2Keys (masklist, list) = Prelude.map (memAddress2key . head) list

memAddressesWithMask (masklist, list) = Prelude.map string2decimal $ allAddresses masklist $ i36tobinary $ memAddress2key $ head list
...
generatePossible (c : cs) resultstrings
  | c == 'X' = generatePossible cs (fmap (++ ['1']) resultstrings ++ fmap (++ ['0']) resultstrings)
