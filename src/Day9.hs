...
findDiffToNumInList list num
  | (num `mod` 2) == 0 =
    intersect list $ diffToNum (filter (/= (num `div` 2)) list) num
  | otherwise = intersect list $ diffToNum list num

isNotSumOf2Nums list n =
  [] == findDiffToNumInList (previous25 n list) (list !! n)
...
isNotSumOf2Numsex list n =
  [] == findDiffToNumInList (previous5 n list) (list !! n)

previous5 n = take 5 . drop (n - 5)

firstNotSumex = example9 !! ind
  where
    ind = head $ filter (isNotSumOf2Numsex example9) [5 .. length example9 - 1]
...
allStretches list ind =
  [takeDrop n m list | n <- [0 .. ind], m <- [0 .. ind], n + m <= ind]

stretchSumsToNum list ind =
  filter (\l -> sum l == list !! ind) $ allStretches list ind

addSmallestLargest list = head sortedlist + last sortedlist
  where
    sortedlist = sort list

encryptionWeakness list ind =
  addSmallestLargest $ head $ stretchSumsToNum list ind
