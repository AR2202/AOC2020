...
mkStore active ind = store checkFunction ind
  where
    checkFunction = checkCube active
...
step (cubes, active) = (newCubes, newActive)
  where
    newCubes = updateStore active $ extend neighboursToCubeState cubes
    newActive = updateActiveCubes active newCubes

updateActiveCubes :: [Coord3] -> Store Coord3 Cube -> [Coord3]
updateActiveCubes active currentCells =
  filter (\x -> (peek x currentCells) == Active) $
  nub $ active ++ activeNeighbours
  where
    activeNeighbours = active >>= neighbours

updateStore currentLiving currentCells = mkStore newLiving ind
  where
    newLiving = updateActiveCubes currentLiving currentCells
...
activeNeighbours4 coord active =
  length . filter (`elem` active) $ neighbours4D coord
...
neighboursToCubeState4 cube active
  ...
  where
    cubeState = checkCube active cube
