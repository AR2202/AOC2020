...
isValid (lowerBound, upperBound, char, password)
  ...
  | otherwise = False
  where
    numOccurrances = length . filter (== char) $ password
...
upperInString string =
  read $ secondOfTriple $ extractUpperBound $
  thirdOfTriple $extractLowerBound string :: Int

passwordConditions string =
  ( lowerInString string
  , upperInString string
  , charInString string
  , passwordInString string)
...
isValidPart2 (firstIndex, secondIndex, char, password) =
  ((password !! (firstIndex - 1)) == char) `xor`
  ((password !! (secondIndex - 1)) == char)
