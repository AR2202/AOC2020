...
testaandb'' =
  parseWithWhitespace (parseAthenB (parseAorB parseA parseB) parseB) "bb"

parseSnd :: (a, String) -> (a, Rule)
parseSnd tuple =
  (fst tuple, fromRight $parseWithWhitespace parseRule' $snd tuple)
...
lookupRule intmap key = eval $fromJust $ IM.lookup key intmap
  where
    eval (Num i)       = lookupRule intmap i
