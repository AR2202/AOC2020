...
part2 filename = do
  ...
  let !validFields =
        map
          (\r ->
             filter
               (allInRange r validTickets)
               [0 .. length (head validTickets) - 1])
          ranges
  let !rowsFields = zip [1 ..] validFields
  let !fields = findFields rowsFields []
  let !departurefields =
        map (head . snd) $filter (\x -> fst x `elem` [1 .. 6]) fields
...
findFields !unidentified !identified =
  findFields newunidentified (newidentified ++ identified)
  where
